...
impl ProfileConfig {
    ...
    pub fn new(name: String, profile_type: ProfileType) -> Self {
        ...
        Self {
            ...
            color: None,
            active: false,
            settings: HashMap::with_capacity(10),
            bookmarks: Vec::with_capacity(0),
...
/// Profile manager (Optimized)
pub struct ProfileManager {
    kernel: Arc<VantisKernel>,
    profiles: Arc<RwLock<HashMap<String, Arc<ProfileConfig>>>>,
...
impl ProfileManager {
    ...
    pub fn new(kernel: Arc<VantisKernel>) -> Result<Self> {
        ...
        let manager = Self {
            kernel,
            profiles: Arc::new(RwLock::new(HashMap::with_capacity(10))),
    ...
    /// Load profiles from disk (Optimized)
    fn load_profiles(&self) -> Result<()> {
        debug!("Loading profiles from disk");

        let mut profiles = HashMap::with_capacity(10);
                            ...
                            if file_name.ends_with(".json") {
                                ...
                                if let Ok(content) = std::fs::read_to_string(&path) {
                                    if let Ok(profile) = serde_json::from_str::<ProfileConfig>(&content) {
                                        profiles.insert(profile.id.clone(), Arc::new(profile));
                                    }
                                }
                            }
                        }
    ...
    pub async fn add_profile(&self, profile: ProfileConfig) -> Result<()> {
        ...
        // Add to memory
        self.profiles.write().await.insert(profile_id.clone(), Arc::new(profile));

        Ok(())
    }

    /// Get profile by ID (Optimized - returns Arc reference)
    pub async fn get_profile(&self, profile_id: &str) -> Option<Arc<ProfileConfig>> {
        self.profiles.read().await.get(profile_id).cloned()
    }

    /// Get all profiles (Optimized - returns Vec<Arc>)
    pub async fn get_all_profiles(&self) -> Vec<Arc<ProfileConfig>> {
        self.profiles.read().await.values().cloned().collect()
    }

    /// Get active profile (Optimized - returns Arc reference)
    pub async fn get_active_profile(&self) -> Option<Arc<ProfileConfig>> {
        let active_id = self.active_profile.read().await.clone()?;
        self.get_profile(&active_id).await
    }

    /// Set active profile (Optimized - reduced lock contention)
    pub async fn set_active_profile(&self, profile_id: &str) -> Result<()> {
        ...
        if let Some(profile) = self.profiles.write().await.get_mut(profile_id) {
            ...
            profile_data.last_used_at = chrono::Utc::now().timestamp_millis();
            self.save_profile(&profile_data)?;
