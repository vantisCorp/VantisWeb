...
/// Profile templates manager (Optimized)
...
impl TemplateManager {
    ...
    pub fn new() -> Self {
        let mut manager = Self {
            templates: HashMap::with_capacity(4),
        };
    ...
    /// Loads default templates (Optimized)
    fn load_default_templates(&mut self) {
        ...
        let work_template = ProfileTemplate {
            ...
            settings: TemplateSettings {
                ...
                keyboard_shortcuts: HashMap::with_capacity(0),
        ...
        let developer_template = ProfileTemplate {
            ...
            description: "Optimized for web development".to_string(),
            category: TemplateCategory::Developer,
            settings: TemplateSettings {
                search_engine: Some("https://www.google.com/search?q=".to_string()),
                homepage: Some("https://developer.mozilla.org".to_string()),
                start_pages: vec![
                    "https://developer.mozilla.org".to_string(),
                    "https://github.com".to_string(),
                    "https://stackoverflow.com".to_string(),
                    "https://caniuse.com".to_string(),
                ],
                keyboard_shortcuts: HashMap::with_capacity(0),
                privacy: PrivacySettings {
                    block_trackers: false,
        ...
        // Insert templates
        self.templates.insert(work_template.name.clone(), work_template);
        self.templates.insert(gaming_template.name.clone(), gaming_template);
        self.templates.insert(privacy_template.name.clone(), privacy_template);
        self.templates.insert(developer_template.name.clone(), developer_template);
    }

    /// Gets a template by name (Optimized - returns reference)
    pub fn get_template(&self, name: &str) -> Option<&ProfileTemplate> {
        self.templates.get(name)
    }

    /// Gets all templates (Optimized - returns Vec<&>)
    pub fn get_all_templates(&self) -> Vec<&ProfileTemplate> {
        self.templates.values().collect()
    }

    /// Gets templates by category (Optimized)
    pub fn get_templates_by_category(&self, category: &TemplateCategory) -> Vec<&ProfileTemplate> {
        self.templates
            .values()
    ...
    pub fn add_template(&mut self, template: ProfileTemplate) -> Result<()> {
    ...
    /// Imports a template from JSON (Optimized)
    pub fn import_template(&mut self, json: &str) -> Result<()> {
        let template: ProfileTemplate = serde_json::from_str(json)?;
        self.add_template(template)?;
        Ok(())
    }

    /// Exports a template to JSON (Optimized)
    pub fn export_template(&self, name: &str) -> Result<String> {
...
mod tests {
    ...
    fn test_template_manager() {
        ...
        assert_eq!(manager.get_all_templates().len(), 4);
    }

    #[test]
    fn test_get_template() {
        let manager = TemplateManager::new();
        let template = manager.get_template("Work");
        assert!(template.is_some());
        assert_eq!(template.unwrap().category, TemplateCategory::Work);
    }
    ...
    fn test_add_template() {
        ...
        let template = ProfileTemplate {
            ...
            settings: TemplateSettings {
                search_engine: None,
                homepage: None,
                start_pages: Vec::with_capacity(0),
                keyboard_shortcuts: HashMap::with_capacity(0),
                privacy: PrivacySettings {
                    block_trackers: false,
                    block_ads: false,
        ...
        manager.add_template(template).unwrap();
        assert_eq!(manager.get_all_templates().len(), 5);
    }
}