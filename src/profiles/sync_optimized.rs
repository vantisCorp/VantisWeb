//! Profile Synchronization (Optimized)
//!
...
/// Profile sync manager (Optimized)
...
impl ProfileSyncManager {
    /// Creates a new profile sync manager (Optimized)
    pub fn new(profiles_dir: PathBuf) -> Self {
        Self {
            ...
            synced_profiles: HashMap::with_capacity(10),
            conflicts: Vec::with_capacity(0),
            profiles_dir,
        }
    }
    ...
    /// Loads local profiles (Optimized)
    ...
    /// Fetches remote profiles from sync server (Optimized)
    fn fetch_remote_profiles(&self, url: &str, api_key: &Option<String>) -> Result<HashMap<String, SyncedProfile>> {
        ...
        // Convert to HashMap (pre-allocated)
        profiles.reserve(response_data.profiles.len());
    ...
    /// Gets sync configuration (Optimized - returns reference)
